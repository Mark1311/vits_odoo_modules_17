...
class ResConfigSettings(models.TransientModel):
    ...
    receipt_logo = fields.Binary("POS Receipt Logo", attachment=True)
    ...
    def set_values(self):
        ...
        param.set_param('res.config.settings.receipt_logo', self.receipt_logo)

        _logger.info(f"[SET] company_logo = {self.company_logo}")
        _logger.info(f"[SET] receipt_logo = {self.receipt_logo}")
    ...
    def get_values(self):
        ...
        res.update(
            ...
            receipt_logo=param.get_param('res.config.settings.receipt_logo')
        )
        _logger.info(f"[GET] receipt_logo = {self.company_logo}")
        _logger.info(f"[GET] receipt_logo = {self.receipt_logo}")
        return res
